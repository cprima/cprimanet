...
gpxParser.prototype.parse = function (gpxstring) {
    ...
    for (let idx in trks){
        ...
        for (let idxIn in trkpts){
            ...
            // Parse extensions like TerrainAnalysis and NutritionStop
            pt.extensions = {};

            let extensionsElem = trkpt.querySelector('extensions');
            if (extensionsElem) {
                let terrainAnalysisElem = extensionsElem.querySelector('TerrainAnalysis');
                if (terrainAnalysisElem) {
                    pt.extensions.TerrainAnalysis = {
                        isBottomOfAscend: keepThis.getElementValue(terrainAnalysisElem, "isBottomOfAscend"),
                        isPeakOfAscend: keepThis.getElementValue(terrainAnalysisElem, "isPeakOfAscend"),
                        ElevationDelta: keepThis.getElementValue(terrainAnalysisElem, "ElevationDelta"),
                        isBeginOfUndulatingTerrain: keepThis.getElementValue(terrainAnalysisElem, "isBeginOfUndulatingTerrain"),
                        isEndOfUndulatingTerrain: keepThis.getElementValue(terrainAnalysisElem, "isEndOfUndulatingTerrain"),
                        CumulativeElevationGain: keepThis.getElementValue(terrainAnalysisElem, "CumulativeElevationGain"),
                        isFlat: keepThis.getElementValue(terrainAnalysisElem, "isFlat"),
                        isPeakOfDescend: keepThis.getElementValue(terrainAnalysisElem, "isPeakOfDescend"),
                        isBottomOfDescend: keepThis.getElementValue(terrainAnalysisElem, "isBottomOfDescend")
                    };
                }

                let nutritionStopElem = extensionsElem.querySelector('NutritionStop');
                if (nutritionStopElem) {
                    pt.extensions.NutritionStop = true;  // Mark as NutritionStop
                }
            }
