...
My modification, also below:

https://www.shadertoy.com/view/wdKBzy
...
vec3 render( in vec3 ro, in vec3 rd, in float time )
{
    ...
    if( t>0.0 )
    {
        ...
        // material color			
        //vec3 mate = vec3(1.0);
        vec3 mate = vec3( (253.0/255.0), (246.0/255.0), (227.0/255.0)); //base3 253 246 227
        //vec3 mate = vec3( (238.0/255.0), (232.0/255.0), (213.0/255.0)); //base2 238 232 213
        //vec3 mate = vec3( (235.0/255.0), (232.0/255.0), (228.0/255.0)); 
        //vec3( (220.0/255.0), (50.0/255.0), (47.0/255.0)) //WBI?
        //id = hash(id); mate *= 0.1+0.9*float((id>>13)&3)/3.0;
        //id = hash(id); mate  = ( ((id>>8)&15)==0 ) ? vec3(0.0,0.0,0.3) : mate;
        //id = hash(id); mate  = ( ((id>>8)&15)==0 ) ? vec3( (60.0/255.0), (123.0/255.0), (183.0/255.0)) : mate; //WBI blue blau
        id = hash(id); mate  = ( ((id>>8)&15)==0 ) ? vec3( (38.0/255.0), (139.0/255.0), (210.0/255.0)) : mate; //solarized blau
        id = hash(id); mate  = ( ((id>>8)&15)==0 ) ? vec3( (133.0/255.0), (153.0/255.0), (0.0/255.0)) : mate; //solarized green
        id = hash(id); mate  = ( ((id>>8)&15)==0 ) ? vec3( (220.0/255.0), (50.0/255.0), (47.0/255.0)) : mate; //solarized red
        id = hash(id); mate  = ( ((id>>8)&15)==0 ) ? vec3( (181.0/255.0), (137.0/255.0), (0.0/255.0)) : mate; //solarized yellow
        ...
        col = mate*pow(vec3(occ),vec3(1.0));
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
	...
	for( int n=0; n<AA; n++ )
	{
        ...
        float time = iTime;
        #endif
        time /= 5.0; //*iMouse.x/iResolution.x;
