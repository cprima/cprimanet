...
function main() {
    ...
    try {
      ...
      Logger.log("test: caught");
    }
    //updateGraphFile() //stopped working 2021-03-17
}
...
function doCbaam() {
    ...
    if (isArmenianworkingday(year, month, day)) {
        //read RSS
...
function doEcb() {
    ...
    var day = myDate.getDate()
    var year = myDate.getFullYear()
    if (isGermanbankworkingday(year, month, day)) {
...
function automateMailchimp() {
    ...
    var EURUSD_pair = setValueOrDefault(dailyValues[0][3]);
    var AMDEUR_rate = setValueOrDefault(dailyValues[1][1], 2);
    var AMDUSD_rate = setValueOrDefault(dailyValues[1][2], 2);
    var EURUSD_rate = setValueOrDefault(dailyValues[1][3], 4);

    //https://developers.google.com/apps-script/reference/utilities/utilities#formatDate(Date,String,String)
    var rateFormattedDate = Utilities.formatDate(rateDate, "GMT", "yyyy-MM-dd'T'HH:mm:ss'Z'");
    var rateShortformattedDate = Utilities.formatDate(rateDate, "Europe/Berlin", "yyyy-MM-dd");
    var nowFormattedDate = Utilities.formatDate(new Date(), "GMT", "yyyy-MM-dd'T'HH:mm:ss'Z'");

    //---------------------------------------------------------------------------------------------campaign

    subject_line = '[EUR/AMD/USD] ' + AMDEUR_pair + ': ' + AMDEUR_rate + ' ' + AMDUSD_pair + ': ' + AMDUSD_rate + ' ' + EURUSD_pair + ': ' + EURUSD_rate;

    var data = {
        "type": "regular",
        "recipients": mailchimp_recipients,
        "settings": { "subject_line": subject_line, "reply_to": "cprior@gmail.com", "from_name": "Christian Prior-Mamulyan", "auto_fb_post": ["110526437402428"] }
    }
    //,"auto_tweet":true

    //,"social_card":{"image_url":"","description":"","title":""}
    ...
    campaign_long_archive_url = JSON.parse(response.getContentText())["long_archive_url"]
    ...
    file_thumbnail_url = JSON.parse(response.getContentText())["thumbnail_url"];
    //Logger.log('full size url: ' + file_full_size_url);

    //var data = {
    //    "social_card":{"image_url":file_full_size_url,"description":subject_line,"title":subject_line}
    //}
    //response = callMailchimpApi(mailchimp_baseurl + 'campaigns/' + campaign_id,
    //    'patch',
    //    data);
    //Logger.log(response.getContentText());
    ...
    var data = {
        "template": {
            ...
            "sections": {
                "title": rateShortformattedDate + " EUR/AMD/USD",
...
message = rateShortformattedDate + " EUR/AMD/USD" + String.fromCharCode(10) + String.fromCharCode(10) + AMDEUR_pair + ": " + AMDEUR_rate + String.fromCharCode(10) + AMDUSD_pair + ": " + AMDUSD_rate + String.fromCharCode(10) + EURUSD_pair + ": " + EURUSD_rate;

    var data = {"message": message, "slug": "", "link": campaign_long_archive_url};
    response = callMailchimpApi(facebook_baseurl + 'feed?access_token=' + facebook_accesstoken,
        'post',
        data);
    Logger.log(response.getContentText());
...
function getHtmlTable(sheetname, rows) {
    ...
    firstpass = true
    //build an array with sanitized values for all but the first row
    values.forEach(function (item, index, array) {
        if (firstpass !== true) {
            //array[index][0] = array[index][0].toISOString().slice(0, 10)
            array[index][0] = Utilities.formatDate(array[index][0], "Europe/Berlin", "yyyy-MM-dd");
            array[index][1] = setValueOrDefault(array[index][1], 2)
            array[index][2] = setValueOrDefault(array[index][2], 2)
            array[index][3] = setValueOrDefault(array[index][3], 4) 
        }
        firstpass = false;
        //Logger.log(item)
    })
    ...
    var htmlTable = values.reduce(function (s, e) {
        if (firstpass == true) { tX = 'th' } else { tX = 'td'; }
        firstpass = false;
        return s += "<tr style=\"" + cssTr + " \"><" + tX + " style=\"" + cssTd + "\">" + e.join("</" + tX + "><" + tX + " style=\"" + cssTd + "\">") + "</" + tX + "></tr>"
...
function callMailchimpApiTest(){
  Logger.log(mailchimp_apikey);
  Logger.log(mailchimp_baseurl);
  var response = callMailchimpApi(mailchimp_baseurl, "get", false)
  Logger.log(response);
}
